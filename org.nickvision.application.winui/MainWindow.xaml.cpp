...
namespace winrt::Nickvision::Application::WinUI::implementation 
{
    ...
    void MainWindow::OnNotificationSent(const NotificationSentEventArgs& args)
    {
        ...
        DispatcherQueue().TryEnqueue([this, args]()
        {
            ...
            else if(args.getAction() == "close")
            {
                ...
                clickToken = BtnInfoBar().Click({ this, &MainWindow::CloseFolder });
            }
    ...
    void MainWindow::OnFolderChanged(const EventArgs& args)
    {
        ...
        if(m_controller->isFolderOpened())
        {
            StatusPageFiles().Description(winrt::to_hstring(std::vformat(_n("There is {} file in the folder.", "There are {} files in the folder.", m_controller->getFiles().size()), std::make_format_args(m_controller->getFiles().size()))));
