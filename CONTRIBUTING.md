...
- Collect information about the bug:
  - Debug information provided by the application
    - From the user menu at the bottom of the sidebar, open Help → Debugging Information and copy the information to the clipboard to paste in your issue.
...
- Please provide as much context as possible and describe the _reproduction steps_ that someone else can follow to recreate the issue on their own system. For good bug reports you should isolate the problem and create a reduced test case.
...
To check your translation file, make sure your system is in the locale of the language you are translating and [locally build and run the app](README.md#building) using `npm run tauri dev`. If all steps were carried out successfully, you should see your translation in action!
...
Application is built using [Tauri](https://tauri.app), pairing a Rust backend with a React/TypeScript frontend. With this stack, Application is built for the Windows, macOS, and Linux operating systems from a single shared codebase.
...
- `src/commands` => Thin [Tauri command](https://tauri.app/develop/calling-rust/) handlers exposed to the frontend over IPC.
- `src/controller.rs`, `config.rs`, `folder.rs`, `info.rs`, `translation.rs` => The business logic and data models of the application.
...
- `components` => The UI of the app, including `dialogs` (modal dialogs), `layout` (layout primitives), `pages` (top-level views), and `ui` (generated [shadcn/ui](https://ui.shadcn.com/) primitives, which should not be hand-edited).
- `hooks` => Reusable React hooks.
- `lib` => React context providers that expose app state and Tauri commands to components (configuration, translation, dialogs, navigation, etc...).
...
- [ESLint](https://eslint.org/) (`npm run lint`) and [Prettier](https://prettier.io/) (`npm run format`) for TypeScript/React code (`src`)

Please run `npm run lint` and `npm run format` before submitting a pull request. Generated code (`src/components/ui`) is excluded from linting/formatting and should not be manually restyled.
