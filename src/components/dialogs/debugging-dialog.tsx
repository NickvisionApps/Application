...
export function DebuggingDialog() {
  ...
  useEffect(() => {
    ...
    async function startup() {
      const fetchedDebuggingInformation = await invoke<string>("get_debugging_information");
    ...
    <Dialog open={true} onOpenChange={(open) => {
      ...
      <DialogContent className="flex max-h-[95vh] flex-col">
        <DialogHeader>
          ...
          <DialogDescription>{_g("Send this information to the developer when reporting issues")}</DialogDescription>
        </DialogHeader>
        {debuggingInformation && (
          <ScrollArea className="min-h-0 flex-1">
            ...
            }}>{debuggingInformation}</Streamdown>
