...
interface TranslationProviderState {
  ...
  _nf: (msgid: string, msgidPlural: string, n: number, args: string[]) => string;
  _p: (msgctxt: string, msgid: string) => string;
  _pf: (msgctxt: string, msgid: string, args: string[]) => string;
  _np: (msgctxt: string, msgid: string, msgidPlural: string, n: number) => string;
  _npf: (msgctxt: string, msgid: string, msgidPlural: string, n: number, args: string[]) => string;
}
...
export function TranslationProvider({children, ...props}: TranslationProviderProps) {
  ...
  const fetchTranslation = useCallback((key: string, cmd: string, args: InvokeArgs) => {
    ...
    invoke(cmd, args).then((value: unknown) => {
  ...
  return (
    <TranslationProviderContext.Provider {...props} value={useMemo<TranslationProviderState>(() => ({
      ...
      _nf: (msgid: string, msgidPlural: string, n: number, args: string[]): string => {
        ...
        fetchTranslation(key, "translate_nf", {msgid, msgidPlural, n, args});
        return fallback;
      },
      _p: (msgctxt: string, msgid: string): string => {
      ...
      _np: (msgctxt: string, msgid: string, msgidPlural: string, n: number): string => {
        ...
        fetchTranslation(key, "translate_np", {msgctxt, msgid, msgidPlural, n});
        return fallback;
      },
      _npf: (msgctxt: string, msgid: string, msgidPlural: string, n: number, args: string[]): string => {
        const fallback = n === 1 ? msgid : msgidPlural;
        ...
        fetchTranslation(key, "translate_npf", {msgctxt, msgid, msgidPlural, n, args});
        return fallback;
      }
